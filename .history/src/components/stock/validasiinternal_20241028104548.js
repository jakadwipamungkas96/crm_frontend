...
function Validasiinternal() {
    ...
    const columnsLsStock = [
        ...
        },
        {
            ...
            selector: row => {
                if (rulesName === "do") {
                    if (row.do_internal_status === 'waiting' && row.do_fisik_status === 'approve') {
                        return <><button type='button' onClick={() => {showApprove(row.id, row.no_spk, row.model, row.no_rangka, row.warna)}} className='btn btn-sm btn-info'><i className="ri ri-more-fill"></i> Validasi DO INTERNAL</button></>
                    } else {
                        return <><button type='button' disabled className='btn btn-sm btn-info'><i className="ri ri-more-fill"></i> Validasi DO INTERNAL</button></>
                    }
                }
            },
            sortable: true,
                                ...
                                <div className="row">
                                    <div className="col-md-6">
                                        ...
                                        <div className="btn-group btn-group-md" role="group" aria-label="Basic example">
                                            <>
                                                <h5>Monitoring SPK - Validasi DO Internal</h5>
                                            </>
                                        </div>
                                    </div>
                                </div>
...
export default Validasiinternal;
