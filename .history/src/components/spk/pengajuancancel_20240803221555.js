...
function Pengajuancancel() {
    ...
    useEffect(() => {
        ...
        const getData = async () => {
            const url = `http://127.0.0.1:8000/api/spk/list/pengajuan?page=${page}&size=${pageSize}`;
            try {
    ...
    const columnsLsStock = [
        ...
        },
        {
            name: 'Aksi',
            selector: row => {
                return <><button type='button' onClick={() => {showApprove(row.id)}} className='btn btn-sm btn-info'><i className="ri ri-more-fill"></i> Validasi Pengajuan</button></>
            },
            sortable: true,
            width: '200px',
        },
