...
function Bucket() {
    ...
    const getHistoryKlaim = async (statusklaim) => {
        const url = `http://127.0.0.1:8000/api/bucket/history?id_cabang=${inputCabang}&statusklaim=${statusklaim}`;
        try {
            const response = await axios.get(url);
    ...
    const columnHistory = [
        ...
        {
            name: 'Claim By Sales',
            // selector: row => (row.id_sales + " - " + row.nama_sales),
            selector: row => {
                if (row.klaim_at !== null) {
                    return (row.id_sales + " - " + row.nama_sales)
                } else {
                    return ""
                }
            },
                                ...
                                <div className="d-flex align-items-center mb-2">
                                    ...
                                    <div className="flex-shrink-0">
                                        <div id="" className='p-2'>
                                            <button onClick={handleOpenHistory} type="button" className={`btn btn-sm btn-primary ${rulesName == 'superadmin' ? "" : ""}`}><i className=" ri-user-search-line"></i> History Klaim</button>
                                        </div>
                                    </div>
                                </div>
                                {loadingTable ? (
