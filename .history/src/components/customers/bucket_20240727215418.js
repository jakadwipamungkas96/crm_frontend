...
function Bucket() {
            ...
            selector: row => {
                if (rulesName == 'sales') {
                    if (row.flagging_check === 1) {
                        ...
                        return <span className={`badge border border-success text-warning`}>
                            Pending Klaim
                        </span>
                    } else {
                        return <span className={`badge border border-success text-secondary`}>
                            Pending Share
                        </span>
                    }
                } else {
    ...
    const getHistoryKlaim = async () => {
        const url = `http://127.0.0.1:8000/api/bucket/history?id_cabang=${inputCabang}`;
        try {
            const response = await axios.get(url);
                                ...
                                <div className="d-flex align-items-center mb-2">
                                    ...
                                    <div className="flex-shrink-0">
                                        <div id="" className='p-2'>
                                            <button onClick={handleOpenHistory} type="button" className="btn btn-sm btn-primary"><i className=" ri-user-search-line"></i> History Klaim</button>
                                        </div>
                                    </div>
                                </div>
                                {loadingTable ? (
