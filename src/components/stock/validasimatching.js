...
function Validasimatching() {
            ...
            selector: row => {
                if (rulesName === "stock") {
                    if (row.matching_status === 'waiting') {
                        return <><button type='button' onClick={() => {showApprove(row.id, row.no_spk, row.model, row.no_rangka, row.warna)}} className='btn btn-sm btn-info'><i className="ri ri-more-fill"></i> Validasi Matching</button></>
                    } else {
                        return <><button type='button' disabled className='btn btn-sm btn-info'><i className="ri ri-more-fill"></i> Validasi Matching</button></>
                    }
                }
            },
            sortable: true,
    ...
    const handleSubmitValidasi = async () => {
        setLoading(true);
        if (arrPengajuan.validasi_matching == "") {
            ...
            try {
                ...
                const response = await axios.post(`https://api.crm.wijayatoyota.co.id/api/spk/validasi/matching`, arrPengajuan);
                ...
                if (response.data.status == "success") {
                    ...
                    window.location.href = "/stock/validasi/matching";
                                ...
                                <div className="row">
                                    <div className="col-md-6">
                                        ...
                                        <div className="btn-group btn-group-md" role="group" aria-label="Basic example">
                                            <>
                                                <h5>Monitoring SPK - Validasi Matching Unit</h5>
                                            </>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <div className="row">
                            <div className="col-lg-12 col-md-12 mb-3">
                                <div className="form-floating">
                                    <select type="text" className="form-control form-control-sm form-select" onChange={handleChangeInput} name="validasi_matching" id="validasi_matching" placeholder="" required>
                                        <option value={""}>-- Pilih Status --</option>
                                        <option value={"match"}>Match</option>
                                        <option value={"unmatch"}>Unmatch</option>
                                    </select>
                                    <label htmlFor="validasi_matching">Silahkan Pilih Status</label>
                                </div>
                            </div>
...
export default Validasimatching;
